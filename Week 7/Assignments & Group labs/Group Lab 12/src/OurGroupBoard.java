public class OurGroupBoard implements Board {
    ...
    public String toString(){
        ...
        for(int i=0;i<this.getRows();i++){
            for(int j=0;j<this.getColumns();j++){
                if(this.board_dat[i][j]!=0)tmp+=String.valueOf(this.board_dat[i][j]);
                else tmp+="_";
            }
            tmp+=String.valueOf('\n');
        }
    ...
    public void add_word(String word, Node p){
        for(int i=0;i<word.length();i++){
            if(p.existence[word.charAt(i)-'a']==null)p.existence[word.charAt(i)-'a']=new Node();
            p=p.existence[word.charAt(i)-'a'];
        }
        p.existence[26]=new Node();
    }
    public void setupNewBoard(String wordlistname,int rows,int columns){
        ...
        for(int i=0;i<word_list.length;i++)if(word_list[i].equals("aa")){pointer=i;break;}
        for(int i=pointer;i<word_list.length;i++)add_word(word_list[i], head);
    ...
    public char getLetterAt(int row,int col){
        ...
        if (c>=0)return board_dat[row][col];
        return 0;
    }
    public boolean Fsearch(String word,Node p){
        Node ph=p;
        for(int i=0;i<word.length();i++){
            ph=ph.existence[word.charAt(i)-'a'];
            if(ph==null)return false;
        }
        if(ph.existence[26]==null) return false;
        return true;
    }
    public boolean check_words(String word){
        return Fsearch(word, head);
    }
        ...
        if(vertical){
            for(int i=0;i<word.length();i++){
                if(check_board_char(this.board_dat, row+i, col)){
                    if(this.board_dat[i+row][col]!=word.charAt(i)) return "0";
                }
                else tmp+=word.charAt(i);
            }
        }
        else{
            for (int i=0;i<word.length();i++){
                if(check_board_char(this.board_dat, row, col+i)){
                    if(this.board_dat[row][col+i]!= word.charAt(i))return "0";
                }
                else tmp+=word.charAt(i);
            }
        }
    ...
    public boolean concate_string_v(char[][] board,boolean v,int ii,String word, int row, int col){
        String tmp="";
        if(!v)tmp=String.valueOf(word.charAt(ii));
        else tmp=tmp+word;
        int row0=row;
        while(row0>0){
            row0--;
            if(!check_board_char(board, row0, col)) break;
        ...
        else return check_words(tmp);
    }
    public boolean concate_string_h(char[][] board,boolean h,int ii, String word, int row, int col){
        String tmp="";
        if(!h)tmp=String.valueOf(word.charAt(ii));
        else tmp=tmp+word;
        int col0=col;
        while(col0>0){
            col0--;
            if(!check_board_char(board, row, col0)) break;
    ...
    public boolean wo_board_check(boolean vertical,String word, int row, int col, char[][] board){
        if(vertical){
            if(!concate_string_v(board,true,0,word, row, col)) return false;
            for(int i=0;i<word.length();i++){
                if(!concate_string_h(board,false,i,word, row+i, col)) return false;
            }
        }
        else{
            if(!concate_string_h(board,true,0,word, row, col))return false;
            for(int i=0;i<word.length();i++){
                if(!concate_string_v(board,false,i,word, row, col+i)) return false;
            }
        }
        return true;
    }
    ...
    public boolean legal_input(boolean vertical, String word,int a,int b){
        ...
        if(vertical){
            if(word.length()+a>this.getRows()-1) return false;
            else return true;
        }
        else{
            if(word.length()+b>this.getColumns()-1) return false;
            else return true;
        }
    }
    public String playWord(Play p,boolean dryRunOnly){
        if(!legal_input(p.isVertical(), p.getWord(),p.getRow(),p.getCol()))return null;
        return_word=word_overlap(p.isVertical(), p.getWord().toLowerCase(),p.getRow(),p.getCol());
        if(return_word.equals("0")) return null;
        if(!wo_board_check(p.isVertical(),p.getWord().toLowerCase() ,p.getRow(), p.getCol(),this.board_dat)) return null;
        else{
            if(!dryRunOnly)input_word(p.isVertical(),this.board_dat, p.getWord().toLowerCase(), p.getRow(), p.getCol());
            return return_word;
        }
    }
}
