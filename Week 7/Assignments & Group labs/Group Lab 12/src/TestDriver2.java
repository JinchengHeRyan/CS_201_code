...
public class TestDriver2 {
	...
	public static int playGame(Board b, List<Player> players) {
		...
		pool.addAll(Arrays.asList("a b c d e x y z".split(" ")));
		...
		while (true) {
			...
			for (int i = 0; i < lettersPlayed.length(); i++) {
				if (!hand.remove("" + lettersPlayed.charAt(i))) {
				  System.out.println("game over - played word using letter not in hand: " + lettersPlayed.charAt(i));
				  return playerIndex; 
				}
			}
	...
	public static void main(String[] args) {
		// TODO create your board object here
		...
		// players.add(new Player(new Group10WordFinder()));
		players.add(new Player(new Group6WordFinder()));
