...
public class SmartBoard_binary implements Board {
    ...
    public void setupNewBoard(String wordListFilename, int rows, int columns) {
        ...
        this.words_list = x.readAll().split("\\s+");
    }
    ...
    public boolean check_insert_word_legality(Play p) {
        ...
        if (p.isVertical()) {
            if (p.getRow() + word.length()  > this.getRows()) return false;
            ...
            if (p.getCol() + word.length()  > this.getColumns()) return false;
    ...
    public boolean search(String word) {
        int low = 0;
        int high = this.words_list.length - 1;
        while (low <= high) {
            int mid = (low + high) / 2;
            if (this.words_list[mid].compareTo(word) < 0) {
                low = mid + 1;
            }
            else if (this.words_list[mid].compareTo(word) > 0) {
                high = mid - 1;
            }
            else if (this.words_list[mid].equals(word)) {
                return true;
            }
        }
        return false;
        ...
        if (wordslist.size() > 0) {
            for (int i = 0; i < wordslist.size(); i++) {
                if (this.must_indict.contains(wordslist.get(i))) {
                    ...
                    if (this.search(wordslist.get(i))) {
